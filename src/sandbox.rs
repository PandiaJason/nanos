...
impl AgentState {
    pub fn record_trace(&mut self, trace: crate::trace::AgentTrace) {
        ...
        crate::dashboard::log_event(format!(
            "[Agent {}] FFI: {} -> {}",
            self.name, trace.action, trace.result
        ));
...
pub fn execute_sandbox_with_replay(
    ...
    info!(
        "Sandbox configured and ready. Boot time: {:.2?}",
        sandbox_boot_elapsed
    );

    // Load and execute the pre-compiled WASM module
    let default_wasm = if std::path::Path::new(
        "nanos-core-agent/target/wasm32-unknown-unknown/release/nanos_core_agent.wasm",
    )
    .exists()
    {
    ...
    if let Some(pos) = wasm_bytes.windows(15).position(|w| w == b"nanos_js_bundle") {
        ...
        info!(
            "Found packaged JavaScript agent bundle inside custom WASM section. Spawning Node.js FFI bridge..."
        );
    ...
    match run_agent.call(&mut store, ()) {
        ...
        Err(e) => {
            ...
            if remaining_fuel == 0 {
                info!(
                    "Agent terminated: fuel budget exhausted (max_steps={} reached).",
                    manifest.resources.max_steps
                );
            } else {
                return Err(e.into());
            }
    ...
    if let Ok(remaining) = store.get_fuel() {
        ...
        info!(
            ...
            used,
            fuel_budget,
            (used as f64 / fuel_budget as f64) * 100.0
        );
    }
...
fn bind_mcp_syscalls(linker: &mut Linker<AgentState>) -> Result<()> {
         ...
         -> i32 {
            ...
            if let Some(replay) = &state_mut.replay {
                if current_step == replay.target_step {
                    info!(
                        "[Replay] Injecting fs_read override for Step {}: {}",
                        current_step, replay.override_observation
                    );
            ...
            memory
                .write(&mut caller, out_ptr as usize, &resp_bytes[..len_to_copy])
                .unwrap();
    ...
    linker.func_wrap(
        ...
        |mut caller: Caller<'_, AgentState>,
         ...
         -> i32 {
            ...
            memory
                .read(&caller, content_ptr as usize, &mut content_bytes)
                .unwrap();
            ...
            state_mut.record_trace(crate::trace::AgentTrace {
                ...
                tokens: "-".to_string(),
                latency,
                result: if response.starts_with("Successfully") {
                    "OK".to_string()
                } else {
                    "Failed".to_string()
                },
    ...
    linker.func_wrap(
        "env",
        "get_manifest_goal",
        |mut caller: Caller<'_, AgentState>, out_ptr: i32, out_max: i32| -> i32 {
            let start_time = std::time::Instant::now();
            let memory = caller.get_export("memory").unwrap().into_memory().unwrap();
        ...
        |mut caller: Caller<'_, AgentState>,
         ...
         -> i32 {
            ...
            let response_text = if !manifest.permissions.network {
                info!(
                    "[Security] 'web_get' blocked network access for url: {}",
                    url
                );
    ...
    linker.func_wrap(
        ...
        |mut caller: Caller<'_, AgentState>, ptr: i32, len: i32| -> i32 {
            ...
            memory
                .read(&caller, ptr as usize, &mut content_bytes)
                .unwrap();
    ...
    linker.func_wrap(
        ...
        |mut caller: Caller<'_, AgentState>,
         ...
         -> i32 {
            ...
            memory
                .read(&caller, ptr as usize, &mut query_bytes)
                .unwrap();
    ...
    linker.func_wrap(
        ...
        |mut caller: Caller<'_, AgentState>,
         ...
         -> i32 {
            ...
            memory
                .read(&caller, prompt_ptr as usize, &mut prompt_bytes)
                .unwrap();
            ...
            if let Some(replay) = &state_mut.replay {
                if current_step == replay.target_step {
                    info!(
                        "[Replay] Injecting LLM response override for Step {}: {}",
                        current_step, replay.override_observation
                    );
                    override_res = Some(replay.override_observation.clone());
                }
            ...
            state_mut.record_trace(crate::trace::AgentTrace {
                ...
                latency,
                result: if response.contains("action") {
                    "JSON OK".to_string()
                } else {
                    "Text".to_string()
                },
    ...
    linker.func_wrap(
        ...
        |mut caller: Caller<'_, AgentState>,
         ...
         -> i32 {
            ...
            memory
                .read(&caller, target_ptr as usize, &mut target_bytes)
                .unwrap();
            ...
            let success = if crate::network::is_node_mode() {
                match crate::network::call_network_bus(
                    "send",
                    serde_json::json!({ "target": target, "msg": msg }),
                ) {
                    Ok(_) => 1,
                    Err(e) => {
    ...
    for line_result in reader.lines() {
        ...
        if let Some(ref r) = replay {
            if (step + 1) == r.target_step {
                info!(
                    "[Replay] Injecting JS FFI override for Step {} (Method: {}): {}",
                    step + 1,
                    method,
                    r.override_observation
                );
            ...
            match method {
                ...
                "web_get" => {
                    ...
                    let res = if !manifest.permissions.network {
                        info!(
                            "[Security] JS FFI 'web_get' blocked network access for url: {}",
                            url
                        );
                ...
                "agent_recv" => {
                    let res = if crate::network::is_node_mode() {
                        ...
                        message_bus
                            .recv(&agent_name)
                            .unwrap_or_else(|| "[No messages in queue]".to_string())
                    } else {
                        "Error: MessageBus not loaded.".to_string()
                    };
                    let latency = start_time.elapsed();
