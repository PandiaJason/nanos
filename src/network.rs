...
pub fn start_orchestrator_server(port: u16, manifest_path: &Path) -> Result<()> {
    ...
    info!("🛰️ Starting Distributed Orchestration Server on port {} for expected agents: {:?}", port, expected_agents);
    ...
    let _server_handle = thread::spawn(move || -> Result<()> {
        for stream in listener.incoming() {
            ...
            thread::spawn(move || {
                if let Err(e) = handle_node_connection(stream, state, &manifest, &expected_agents) {
    ...
    loop {
        ...
        let all_exited = expected_agents.iter().all(|name| lock.finished_agents.contains_key(name));
        if all_exited {
            info!("🎉 All expected agents have completed their goals. Shutting down distributed fleet server.");
...
fn handle_node_connection(
    ...
    let agent_name = match reg_msg {
        BusMessage::Register { name } => name,
        _ => return Err(anyhow::anyhow!("First message must be Register")),
    };
    
    if !expected_agents.contains(&agent_name) {
        return Err(anyhow::anyhow!("Unregistered agent node '{}' tried to connect", agent_name));
    }
    ...
    // Register active stream
    {
        ...
        lock.active_connections.insert(agent_name.clone(), stream.try_clone()?);
    }
    
    // 2. Fetch specific manifest config for agent
    let spec = manifest.agents.as_ref().unwrap().iter().find(|a| a.name == agent_name).unwrap();
    ...
    while reader.read_line(&mut line)? > 0 {
        ...
        match msg {
            BusMessage::Send { target, msg } => {
                info!("✉️ Routing message from [{}] to [{}]: {}", agent_name, target, msg);
                let mut lock = state.lock().unwrap();
            ...
            BusMessage::Recv => {
                ...
                let content = if queue.is_empty() { None } else { Some(queue.remove(0)) };
                writeln!(stream, "{}", serde_json::to_string(&BusMessage::Msg { content })?)?;
                stream.flush()?;
            }
            BusMessage::Exit { status, summary } => {
                info!("🏁 Agent node [{}] finished with status '{}': {}", agent_name, status, summary);
                let mut lock = state.lock().unwrap();
...
pub fn start_agent_node(connect_addr: &str, agent_name: &str) -> Result<()> {
    info!("🚀 Connecting Agent Node [{}] to distributed message bus at {}...", agent_name, connect_addr);
    let stream = TcpStream::connect(connect_addr)
        .with_context(|| format!("Failed to connect to orchestrator server at {}", connect_addr))?;
        
    let mut reader = BufReader::new(stream.try_clone()?);
    
    // 1. Send registration
    let reg_msg = BusMessage::Register { name: agent_name.to_string() };
    ...
    let (goal, _tools, manifest) = match init_msg {
        BusMessage::Init { goal, tools, manifest } => (goal, tools, manifest),
    ...
    if let Some(stream) = take_node_socket() {
        ...
        let exit_msg = BusMessage::Exit {
            status: if run_res.is_ok() { "success".to_string() } else { "failed".to_string() },
            summary: format!("{:?}", run_res.as_ref().map(|_| "Execution completed cleanly").unwrap_or("Execution failed")),
        };
...
static NODE_SOCKET: std::sync::OnceLock<Mutex<Option<Arc<Mutex<TcpStream>>>>> = std::sync::OnceLock::new();
...
mod tests {
    ...
    fn test_network_message_bus() -> Result<()> {
        ...
        let server_thread = thread::spawn(move || -> Result<()> {
            ...
            while reader.read_line(&mut line)? > 0 {
                ...
                match msg {
                    ...
                    BusMessage::Recv => {
                        let resp = BusMessage::Msg { content: Some("secret".to_string()) };
        ...
        let reg_msg = BusMessage::Register { name: "test-agent".to_string() };
        ...
        let send_res = call_network_bus("send", serde_json::json!({
            "target": "writer",
            "msg": "hello"
        }))?;
