            ...
            "local" => {
                ...
                thread::spawn(move || {
                    ...
                    for req in rx {
                        ...
                        info!(
                            "Prompt tokens: {:?}",
                            &tokens[..std::cmp::min(20, tokens.len())]
                        );
                        ...
                        while generated_tokens < max_gen_tokens {
                            let candidates_vec: Vec<_> =
                                ctx.candidates_ith(batch.n_tokens() - 1).collect();
                            let has_nan = candidates_vec.iter().any(|c| c.logit().is_nan());
                            let min_logit = candidates_vec
                                .iter()
                                .map(|c| c.logit())
                                .fold(f32::INFINITY, f32::min);
                            let max_logit = candidates_vec
                                .iter()
                                .map(|c| c.logit())
                                .fold(f32::NEG_INFINITY, f32::max);
                            info!(
                                "Logits diagnostic: has_nan={}, min={}, max={}, count={}",
                                has_nan,
                                min_logit,
                                max_logit,
                                candidates_vec.len()
                            ...
                            let piece = loop {
                                match model.token_to_piece_bytes(best_token, buf_size, true, None) {
                                    ...
                                    Err(e) => {
                                        info!(
                                            "token_to_piece_bytes error for token {:?}: {:?}",
                                            best_token, e
                                        );
                                        break String::new();
                                    }
